...
$(document).ready(function() {
    ...
    $(window).resize(function() {
        ...
        if (do_width > 1024) {
			...
			$( ".menu_cong_header a:nth-child(4) .title_cong_menu" ).replaceWith( "<div class='title_cong_menu w_100' style='margin-left: 6px'>Bán thời gian</div>" );
...
$('#btn_select_luong').click(function() {
    ...
    if($('#search_box_header_ntd .box_mucluong .jq-dropdown-open').length > 0){
        $('#search_box_header_ntd .box_mucluong .jq-dropdown-open').removeClass('jq-dropdown-open');
    }
...
$('#btn_select_kinh_nghiem').click(function() {
    ...
    if($('#search_box_header_ntd .box_kinhnghiem .jq-dropdown-open').length > 0){
        $('#search_box_header_ntd .box_kinhnghiem .jq-dropdown-open').removeClass('jq-dropdown-open');
    }
...
$('#btn_search_time').click(function() {
    ...
    $('#filter_mucluong2').hide();
    if($('#search_box_hosoungvien .box_mucluong .jq-dropdown-open').length > 0){
        $('#search_box_hosoungvien .box_mucluong .jq-dropdown-open').removeClass('jq-dropdown-open');
    }

    if($('#search_box_full .box_mucluong .jq-dropdown-open').length > 0){
...
$('#btn_search_time3').click(function() {
    ...
    $('#user_menu_filter_kinhnghiem1').hide();
    if($('#search_box_hosoungvien .box_kinhnghiem .jq-dropdown-open').length > 0){
        $('#search_box_hosoungvien .box_kinhnghiem .jq-dropdown-open').removeClass('jq-dropdown-open');
    }
    if($('#search_box_full .box_kinhnghiem .jq-dropdown-open').length > 0){
...
$('#btn_search_trinh_do').click(function() {

    var numSelected = $('#cols-right .box_trinh_do .icheckbox_square-blue.checked').length;
    if(numSelected > 0) {
        if(numSelected < 10) {
            numSelected = "0" + numSelected;
        }
        var text = '<strong>Đang chọn ' + numSelected +' Trình độ</strong>';
    } else {
        var text = 'Tất cả Trình độ';
    }
    $('#cols-right .box_trinh_do .drop_filter_top').html(text);
    $('#user_menu_filter_trinh_do').hide();
    if($('#search_box_hosoungvien .box_trinh_do .jq-dropdown-open').length > 0){
        $('#search_box_hosoungvien .box_trinh_do .jq-dropdown-open').removeClass('jq-dropdown-open');
    }
    if($('#search_box_full .box_trinh_do .jq-dropdown-open').length > 0){
        $('#search_box_full .box_trinh_do .jq-dropdown-open').removeClass('jq-dropdown-open');
    }
    // $('#cols-right .box_kinhnghiem .drop_filter_top').trigger('click');
    $('.filter_trinh_do').hide();
    dim_color_other_element($(this));
});
$('#btn_search_trinh_do_header').click(function() {

    var numSelected = $('.box_trinh_do_header .icheckbox_square-blue.checked').length;
    ...
    $('.box_trinh_do_header .drop_filter_top').html(text);
    $('#user_menu_filter_trinh_do').hide();
    if($('.box_trinh_do_header .jq-dropdown-open').length > 0){
        $('.box_trinh_do_header .jq-dropdown-open').removeClass('jq-dropdown-open');
    }
    if($('.box_trinh_do_header .jq-dropdown-open').length > 0){
        $('.box_trinh_do_header .jq-dropdown-open').removeClass('jq-dropdown-open');
    }
...
$('#btn_search_nganhnghe').click(function() {
    ...
    if(numSelected > 0) {
        ...
        var text = '<strong>Đang chọn ' + numSelected +' Ngành nghề</strong>';
    } else {
...
// click mouse outside element
$(document).mouseup(function (e) {
    if ($(e.target).closest(".loggedin ").length === 0) {
        $('.main-menu ul li.loggedin').find('.popup_down').hide();
    }
});
...
setTimeout(function(){
    $('.radio-warning-login').on('ifChecked', function () {
        var status = $(this).val();
        $.ajax({
            url: url_update_status_warning_login,
            type: "POST",
            data: {
                status: status
            },
...
function getTrangHoSoUngVien(type, url, value,ttd_id ) {
    var ttd_id = $('#mucluong_mongmuon_id').val();
    window.location.href = url+'?'+type+'='+value+ '&ttd='+ttd_id;
}
