...
$(document).ready(function() {
    ...
    // Bấm đổi loại box ngành nghề o trang viec-lam/viec-lam-theo-nganh-nghe Sắp xếp theo: Ngành hot | ABC
    $('.tab_nganhnghe_box').click(function () {
        var id_tab = $(this).attr('rel');
        $('.tab_nganhnghe_box').removeClass('text_grey').removeClass('text_blue').addClass('text_grey');
        $(this).removeClass('text_grey').addClass('text_blue');
        $(this);
    ...
    // Bấm đổi loại box ngành nghề o trang viec-lam/viec-lam-theo-nganh-nghe Sắp xếp theo: Ngành hot | ABC
    $('.tab_nganhnghe_box_right').click(function () {
        var id_tab = $(this).attr('rel');
        $('.tab_nganhnghe_box_right').removeClass('text_grey').removeClass('text_blue').addClass('text_grey');
        $(this).removeClass('text_grey').addClass('text_blue');
        $(this);
        $('.box_nganhnghe_right').removeClass('hide').addClass('hide');
        $('#'+id_tab).removeClass('hide');
    });
...
//    });
    ...
    $("#viec_tinh .bt_more_ola").click(function(){
        ...
        $("#viec_tinh .hide_limit").show();
    });

    //Thay doi mau cho khi selected
    $('.box_select_filter_reg').each(function (){
        var id = 's2id_'+ $(this).attr('id');
        var val = $(this).val();
        change_text_color_selected(id, val);
    });
...
function loadAjaxContent (container, url, page) {
    $.ajax({
        type: 'GET',
        url: url,
        success: function (data) {
            ...
            var container_arr = new Array('box_vieclam_xacthuc');
            if (container_arr.indexOf(container) != -1) {
                if (page) {
...
$(document).ready(function() {
    ...
    $('.select-style-new').each(function(index, el) {
        var e_id = $(this).attr('id');
        if ($(this).val() == ''){
            $('#s2id_' + e_id).find('.select2-choice').css({'color':'#959595', 'font-weight':''});
        } else {
            $('#s2id_' + e_id).find('.select2-choice').css({'color':'#363636', 'font-weight':'500'});
        }
    });
    $('.select-style-new').each(function(index, el) {
        var e_id = $(this).attr('id');
    ...
    $('.select-style-fix').each(function(index, el) {
        var e_id = $(this).attr('id');
        if ($(this).val() == ''){
            $('#s2id_' + e_id).find('.select2-choice').css({'color':'#959595', 'font-weight':''});
            $('#s2id_' + e_id).find('.select2-choice').find('.select2-chosen').css({'color':'#959595', 'font-weight':''});
        } else {
            $('#s2id_' + e_id).find('.select2-choice').css({'color':'#363636', 'font-weight':'500'});
            $('#s2id_' + e_id).find('.select2-choice').find('.select2-chosen').css({'color':'#363636', 'font-weight':'500'});
        }
    });
    $('.select-style-fix').each(function(index, el) {
        var e_id = $(this).attr('id');
    ...
    $('.select-style-new-two').each(function(index, el) {
        var e_id = $(this).attr('id');
        if ($(this).val() == ''){
            $('#s2id_' + e_id).find('.select2-choice').find('.select2-chosen').css({'color':'#959595', 'font-weight':''});
        } else {
...
function show_bangcap_edit (id) {
    if (!$("#bangcap-edit-form-"+id).exists()) {
        ...
        //An nut them
        $('#add_bangcap_chungchi').attr('style', 'display:none');
        ...
        $.ajax({
            ...
            success: function(data, textStatus, jqXHR) {
                if (data.error == 0) { // thanh cong
                    ...
                    //Hien nut them bang cap

                    $('#add_bangcap_chungchi').removeAttr("style");
                    //Hien lai form them neu ko con danh sach bang cap nao
                    if($('.new-bangcap').length < 1)
                    {
                        $('#add_bangcap_chungchi').click();
                    }
                }
                else {
...
function show_ngoaingu_edit (id) {
    if (!$("#ngoaingu-edit-form-"+id).exists()) {
        ...
        //Hien nut them ngoai ngu
        $('#show-add-ngoai-ngu').attr("style","display:none;");
        $('#div-frm-ngoaingu').attr("style","display:none;");
        ...
        $.ajax({
            ...
            success: function(data, textStatus, jqXHR) {
                if (data.error == 0) { // thanh cong
                    ...
                    //Hien nut them ngoai ngu
                    $('#show-add-ngoai-ngu').removeAttr("style");
                    if($('.new-ngoaingu').length < 1)
                    {
                        $('#show-add-ngoai-ngu').click();
                    }
...
{
    //Start Ẩn thông tin thêm
    $('#addbkinhngiem2_new').attr('style', 'display:none');
    $('#div-frm-kinhnghiem').addClass("display_none").fadeOut();
    $('#div-frm-kinhnghiem .error_reg_mess').html('').addClass('display_none');
    var chk = $('input#chk_c_cong_viec_hien_tai');
    chk.val(1);
    chk.iCheck('uncheck');
    //End Ẩn thông tin thêm
        ...
        $.ajax({
            ...
            success: function(data, textStatus, jqXHR) {
                if (data.error == 0) { // thanh cong
                    ...
                    //Hien nut them kinh nghiem
                    $('#addbkinhngiem2_new').removeAttr("style");
                    if($('.new-kinhnghiem').length < 1)
                    {
                        $('#addbkinhngiem2_new').click();
                    }
...
function show_kynang_edit (id) {
    //An them ky nang
    $('#addbkynang2_new').attr('style', 'display:none');
...
function show_thamkhao_edit (id) {
    //An nguoi tham khao
    $('#addthamkhao2_new').attr('style', 'display:none');
...
function delete_thamkhao (id) {
    //Hien nut them tham khao
    $('#addthamkhao2_new').removeAttr("style");
        ...
        $.ajax({
            ...
            success: function(data, textStatus, jqXHR) {
                if (data.error == 0) { // thanh cong
                    ...
                    if($('.new-thamkhao').length < 1)
                    {
                        $('#addthamkhao2_new').click();
                    }
...
//Thay doi chon loai ngoai ngu
function change_chon_ngoai_ngu(data)
{
    if (data.value == 'OTHER') {
        $('#div_ngonngukhac').fadeIn();
        $('#div_ngonngukhac').removeAttr("style");
    }else{
...
function save_tong_quan(id, id_ttd_ungvien, url_sync) {
    ...
    $("#btnSaveHoSoTongQuan").button('loading');
    var valid = false;
    valid = checkValidHoSo();
    if (!valid){
        $("#btnSaveHoSoTongQuan").prop('disabled', false);
        $("#btnSaveHoSoTongQuan").button('reset');
        return false;
    }else{
        ...
        $.ajax({
            ...
            success: function(data, textStatus, jqXHR) {
                if (data.error == 0) { // thanh cong
                    // Thành Công
                    //nếu có sync cv thi post sync
                    if(data.status_sync_tintimviec_to_cv == 0 && data.cv_id_sync != ""){
                        var c_tieu_de_cv = $('#c_tieu_de').val();
                        var data_html = "";
                        $.get(url_sync+data.cv_id_sync+'?edit_hstb=1',function (html) {
                            data_html = html;
                            if(data_html != "") {
                                $.ajax({
                                    url: '/cv/sync_tintimviec_to_cv',
                                    type: 'POST',
                                    data: {c_tieu_de_cv: c_tieu_de_cv, data_html: data_html},
                                    dataType: 'json'
                                });
                            }
                        });
                    ...
                    $('#btnChinhSuaTongQuan').click(function(event) {
                        ...
                        // $('#c_muc_tieu_nghe_nghiep').val($('#c_muc_tieu_nghe_nghiep').val().substring(0, 1).toUpperCase() + $('#c_muc_tieu_nghe_nghiep').val().substring(1).toLowerCase());
                    });
...
function onclick_check_user_approve(loai_tai_khoan, trang_thai, type_create, url)
{
    ...
    if(loai_tai_khoan == 0)
    {
        if(type_create == 'ttv-edit')
            ...
            text_alert = 'Tài khoản của bạn đang không được duyệt. Vui lòng cập nhật lại tài khoản để tiếp tục chỉnh sửa CV trang trí.';
        else
...
//Hien thi mau sac khi select change
$('.box_select_filter_reg').on("change", function (e) {
    var id = 's2id_'+ $(this).attr('id');
    var val = $(this).val();
    change_text_color_selected(id, val);
});

function change_text_color_selected(id, val)
{
    if(val == 0)
        $('#'+id).find('.select2-chosen').attr('style','color:#c1bebe');
    else
        $('#'+id).find('.select2-chosen').attr('style','color:#757575');
}
//Cap nhat lai thong tin trong textarea
function CKupdate(){
...
//Reset editor
function reset_ckeditor(content){
    for ( instance in CKEDITOR.instances ){
        CKEDITOR.instances[instance].setData(content);
    }
}
//function nguoi tim viec theo doi nha tuyen dung
function seeker_follow_employer(seeker_id, employer_id, status)
{
    var label = seeker_id + '-' + employer_id;
    if (false) {
        if (status == 1) {
            ga('send', 'event', 'follow_NTD', 'follow', label);
        } else if (status == 2) {
            ga('send', 'event', 'follow_ntd', 'click', 'topNTD')
        } else {
            ga('send', 'event', 'follow_NTD', 'unfollow', label);
        }
    }

    // var text = 'Bạn đồng ý theo dõi Nhà tuyển dụng này?';
    // if(status == 0)
    //     text = 'Bạn đồng ý hủy theo dõi Nhà tuyển dụng này?';

    if(true){
        $.ajax({
            type: 'post',
            dataType : 'json',
            url: '/nhatuyendung/seeker_follow_employer',
            data: {seeker_id:seeker_id, employer_id:employer_id, status:status},
            success: function(data, textStatus, jqXHR) {
                // thanh cong
                if (data.error == '0') {
                    location.reload();
...
//function nguoi tim viec theo doi nha tuyen dung
function seeker_follow_employer_toiuu(seeker_id, employer_id, status)
{
    var label = seeker_id + '-' + employer_id;
    if (false) {
        if (status == 1) {
    ...
    if(true){
        $.ajax({
            ...
            success: function(data, textStatus, jqXHR) {
                // thanh cong
                if (data.error == '0') {
                    $('.note_tb').attr('style', 'box-shadow:none !important;background: #dff0d8;padding: 8px 14px;margin-bottom: 5px;height:66px !important;margin-top: 10px;border-radius: 2px !important;border: 0px !important;');
                    $('.text-success-theodoi').html('<p style="text-align: left;margin-top: 5px;color: #3c763d;font-weight: 400;">Đăng ký nhận thông báo khi Nhà tuyển dụng có việc làm mới thành công. Bạn có thể vào quản lý <a style="color: #3c763d;" target="_blank" href="/danh-sach-theo-doi-nha-tuyen-dung.html"><span style="font-weight:500 !important">Nhà tuyển dụng đang theo dõi</span></a> để xem chi tiết.</p>');
                    $('.follow_employer').html('<div class="dropdown"><a data-toggle="dropdown" class="btn btn_is_following " aria-expanded="true" id="dropdownMenuButton" ><span class="line-icon"><i class="icon_follow icon_position"></i><span class="pl_28 text_follow">Đang theo dõi</span></span></a><div class="dropdown-menu dropdown-menu-follow font11" aria-labelledby="dropdownMenuButton"><div><i class="icon-cheked"></i> <a class="dropdown-item text_following" href="#" > Đang theo dõi </a> </div> <div> <a class="dropdown-item pl_unfollow " href="#" onclick="seeker_follow_employer('+seeker_id+', '+employer_id+', \'0\');">Hủy theo dõi</a> </div> </div></div>');
                }
            },
            error: function(jqXHR, textStatus, errorThrown) {
...
function open_popup_show_complain_seeker(seeker_id) {
    if (seeker_id && seeker_id != "") {
        $("#popup_ntv_register").load("/nhatuyendung/popup_complain_seeker?seeker_id="+seeker_id, function() {
            $("#overlay_popup, #popup_component").show();
            var height_wd = $( window ).height()/2;
            var height_p  = $( "#popup_content" ).height()/2;
            var top_p     = height_wd -height_p;
...
$('.upload_hosodinhkem').bind('change', function() {

    var file_size = $('.upload_hosodinhkem')[0].files[0].size;
    if(file_size > 2097152)
    {
        var message = 'Vui lòng kiểm tra lại dung lượng file! Dung lượng tối đa cho phép là 2MB' ;
        $('#error_file_dinh_kem').text(message);
        $('#error_file_dinh_kem').removeClass("display_none").addClass('colorRegRed').addClass('fs14');
        $('#cols-right').scrollTo2("#error_file_dinh_kem");
        return false;
    }
    var datapost = new FormData($("#frm-hoso")[0]);
    datapost.append( 'file', $('.upload_hosodinhkem')[0].files[0] );
    $.ajax({
        type: 'post',
        dataType : 'json',
        url: '/tintimviec/checkUploadFileDinhKem',
        data: datapost,
        mimeType:"multipart/form-data",
        processData: false,
        contentType: false,
        success: function(data, textStatus, jqXHR) {
            if (data.error == 0) { // thanh cong
                $('#error_file_dinh_kem').addClass('display_none');
            } else {
                var focus = false;
                ...
                for (key in data.message) {
                    ...
                    if (key == 'c_muc_luong_toi_thieu') {
                        $('#salary_words').html('').addClass('display_none');
                    }
                    if (!focus) {
                        $('#cols-right').scrollTo2("#error_"+ key);
                        focus = true;
                    }
                }
                if (data.main_message != null) {
        ...
        error: function(jqXHR, textStatus, errorThrown) {
            alert('File upload quá giới hạn server 100MB. Vui lòng thử lại. (#' + jqXHR.status + ': ' + textStatus + ')');
        }
    });
...
function show_popup_ntv_khong_duyet_update(text_alert,ly_do,id)
{
    $(window).scrollTop(0);
    var padding_h = $('.regis_header').height();
    var padding_f = $('.regis_footer').height();
    ...
    $("#popup_confirm").load("/common/popup_check_approve_ntv_update",{text_alert:text_alert,ly_do:ly_do,id:id}, function () {
        ...
        if(id == 2){
            $("#btnXoaHoSo").click(function(){
                window.location.href = "/ho-so-dinh-kem.html";
            });
        } else{
            if(id == 1){
                $("#btnXoaHoSo").click(function(){
                    window.location.href = "/ho-so-tung-buoc.html";
                });
            }else {
                $("#btnXoaHoSo").click(function(){
                    window.location.href = "/taikhoan/profile";
                });
            }
        }
